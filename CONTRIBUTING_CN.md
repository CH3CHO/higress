...
建议使用 `asdf` 或 `mise` 之类的版本管理工具读取仓库中的版本文件：

- 仓库根目录 `.go-version` / `.tool-versions` 对应主工程，Go 版本为 `1.22.9`
- `plugins/wasm-go` 目录下的 `.go-version` / `.tool-versions` 对应 Wasm 插件开发，Go 版本为 `1.24.4`

如果你已经安装了对应的版本管理工具，进入不同目录时会自动切换到匹配的 Go 版本。
